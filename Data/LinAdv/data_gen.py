...
for it in range(PARAMS.Nt-1):
    ...
    u_data[:,it+1,1:-1] = u_data[:,it,1:-1] \
    			          - vel*0.5*dt/h*(u_data[:,it,2::] - u_data[:,it,0:-2]) \
    			          +dt*(visc/h**2 + 0.5*abs(vel)/h)*(u_data[:,it,2::] - 2*u_data[:,it,1:-1] + u_data[:,it,0:-2])  
    u_data[:,it+1,0]    = u_data[:,it,0] \
    			          - vel*0.5*dt/h*(u_data[:,it,1] - u_data[:,it,-2]) \
    			          +dt*(visc/h**2 + 0.5*abs(vel)/h)*(u_data[:,it,1] - 2*u_data[:,it,0] + u_data[:,it,-2])	
...
np.save(f"{datadir}/u_data.npy",u_data)
